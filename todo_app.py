...
/* grouped rows: repeated name is invisible until hovered or focused */
[class*="_sub_"] div[data-testid="stColumn"]:first-child input { color: transparent !important; }
[class*="_sub_"] div[data-testid="stColumn"]:first-child:hover input,
[class*="_sub_"] div[data-testid="stColumn"]:first-child input:focus { color: #1B1B1F !important; }
...
hr { margin: 0.35rem 0 !important; border-color: #EDEDEA !important; }
...
with st.form("new_task", clear_on_submit=True, border=False):
    a_who, a_job, a_due, a_btn = st.columns([1.7, 3.6, 1.7, 1.0], vertical_alignment="bottom")
...
def group_open(tasks: list[dict]) -> list[tuple[dict, bool]]:
    """Insertion order, but a task whose Who matches an earlier open task
...
rows = group_open(open_tasks) + [(t, False) for t in done_tasks]

# ---------- table ----------
WIDTHS = [1.7, 3.6, 1.5, 1.1, 0.5]
...
def render(todo: dict, hide_name: bool = False) -> None:
    tid = todo["id"]
    ...
    with st.container(key=f"row_{state}_{'sub' if hide_name else 'top'}_{tid}"):
        ...
        if c_del.button("✕", key=f"del_{tid}", type="tertiary", help="Delete",
