...
/* row states */
[class*="st-key-row_done"] input { color: #B0B0B3 !important; text-decoration: line-through; }
...
/* status toggle */
[class*="st-key-stat_"] button { width: 100%; min-height: 2.35rem !important; font-weight: 600 !important;
...
div[data-testid="stButton"] button[kind="tertiary"] {
    ...
    min-width: 0 !important; width: 100%; border: none !important; min-height: 2.35rem !important; }
...
def render(todo: dict) -> None:
    ...
    with st.container(key=f"row_{state}_{tid}"):
        ...
        with c_stat.container(key=f"stat_{'yes' if done else 'no'}_{tid}"):
            if st.button("Yes" if done else "No", key=f"tog_{tid}"):
