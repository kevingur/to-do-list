...
div[data-testid="stDialog"] div[role="dialog"] { border-radius: 12px; max-width: 280px; padding: 1rem 1.2rem 1.2rem; }
div[data-testid="stDialog"] h1, div[data-testid="stDialog"] h2 { font-size: 1rem !important; font-weight: 600; }
...
def group_rows(tasks: list[dict]) -> list[tuple[dict, bool]]:
    ...
    is placed right after that group. Done tasks stay where they are. Second value = True when the name
...
def confirm_delete(todo_id: str, label: str) -> None:
    c_cancel, c_ok = st.columns(2)
    if c_cancel.button("Cancel", use_container_width=True):
        st.rerun()
    if c_ok.button("Delete", type="primary", use_container_width=True):
        delete_todo(todo_id, label)
        st.rerun()
